...
public:      // funcs
  Node(int _x, int _y, int _z) : x(_x), y(_y), z(_z), w( 3), listptr( NULL) {
     next=NULL;
  }
  ~Node();
...
public:      // funcs
  ...
  enum Kind { ASUBCLASS, ANOTHERSUBCLASS, NUM_KINDS };
  virtual Kind kind() const = 0;

  static char const * const kindNames[NUM_KINDS];
  char const *kindName() const { return kindNames[kind()]; }

  DECL_AST_DOWNCASTS(ASubclass, ASUBCLASS)
  DECL_AST_DOWNCASTS(AnotherSubclass, ANOTHERSUBCLASS)

  virtual NodeList* clone() const=0;
...
class ASubclass : public NodeList {
public:      // data
  int x;

public:      // funcs
  ASubclass(FakeList <Node >* _list, int _x) : NodeList(_list), x(_x) {
  }
  virtual ~ASubclass();

  virtual Kind kind() const { return ASUBCLASS; }
  enum { TYPE_TAG = ASUBCLASS };

  virtual void debugPrint(ostream &os, int indent, char const *subtreeName = "tree") const;
  virtual void xmlPrint(ostream &os, int indent) const;
  virtual void traverse(ExampleVisitor &vis);

  virtual ASubclass *clone() const;

};

class AnotherSubclass : public NodeList {
public:      // data
  int x;

public:      // funcs
  AnotherSubclass(FakeList <Node >* _list, int _x) : NodeList(_list), x(_x) {
  }
  virtual ~AnotherSubclass();

  virtual Kind kind() const { return ANOTHERSUBCLASS; }
  enum { TYPE_TAG = ANOTHERSUBCLASS };

  virtual void debugPrint(ostream &os, int indent, char const *subtreeName = "tree") const;
  virtual void xmlPrint(ostream &os, int indent) const;
  virtual void traverse(ExampleVisitor &vis);

  virtual AnotherSubclass *clone() const;
...
public:      // funcs
  ...
  enum Kind { SUB1, SUB2, SUBWITHDEFAULT, SUB3, SUB4, NUM_KINDS };
  virtual Kind kind() const = 0;
...
public:      // funcs
  Sub3(int _x, Super *_s1, Sub2 *_s2, int _q) : Super(_x), s1(_s1), s2(_s2), q(_q) {
  }
  virtual ~Sub3();
...
class Sub4 : public Super {
public:      // data

public:      // funcs
  Sub4(int _x) : Super(_x) {
  }
  virtual ~Sub4();

  virtual Kind kind() const { return SUB4; }
  enum { TYPE_TAG = SUB4 };
...
public:      // funcs
  ANewClass(int _foo) : foo(_foo) {
  }
  ~ANewClass();

  char const *kindName() const { return "ANewClass"; }

  ANewClass *clone() const;
...
public:      // funcs
  ...
  virtual bool visitANewClass(ANewClass *obj);
  virtual void postvisitANewClass(ANewClass *obj);
